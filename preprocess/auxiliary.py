...
# 生成诊断代码之间的邻接矩阵adj，表示不同诊断代码在病人的住院记录中共同出现的频率。
# 最终返回的是经过归一化和阈值过滤后的邻接矩阵
...
# 为每个病人每次住院记录中的诊断代码找到在邻接矩阵中相邻的邻居代码，并标记出来
...
# 将每个病人的每次住院记录中的诊断代码分为三类
# m1:当前住院记录中与前一次住院记录相同的代码。
# m2:当前住院记录中与前一次住院记录的邻居代码相同的代码。
# m3:当前住院记录中既不是前一次住院记录的代码也不是其邻居代码的代码。
def divide_middle(code_x, neighbors, lens):
    ...
    print('\r\t%d / %d' % (n, n))
    return divided  #divided形状为(n, max_admission_num, code_num, 3) 的布尔数组
